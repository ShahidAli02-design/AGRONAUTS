...
const handleQualityGrading = async (req: Request, res: Response) => {
  const { batchId, image, crop, quantity, localMetrics } = req.body;
  ...
  try {
    const qualityReport = await analyzeProduceQuality(targetBatchId, image, cropName, qty, localMetrics);
    ...
    if (err instanceof GradingInputError) {
      return res.status(422).json({ success: false, error: err.message, notForSale: err.notForSale, detected: err.detected });
    }
    res.status(500).json({ success: false, error: err.message });
  }
};
...
apiRouter.post('/voice/ask', async (req: Request, res: Response) => {
  ...
  if (typeof query !== 'string' || !query.trim() || !isTranslateTarget(lang)) {
    return res.status(400).json({ success: false, error: 'Expected { query: string, lang: "en" | "mr" | "hi" }' });
  }
  const answer = await answerVoiceQuery(query, lang);
  if (!answer) return res.json({ success: false, error: 'AI assistant unavailable' });
  res.json({ success: true, ...answer });
});

// Site-wide UI translation used by the language switcher (src/lib/auto-translate.tsx).
apiRouter.post('/translate', async (req: Request, res: Response) => {
  const { target, texts } = req.body || {};
  if (!isTranslateTarget(target) || !Array.isArray(texts)) {
    return res.status(400).json({ success: false, error: 'Expected { target: "en" | "mr" | "hi", texts: string[] }' });
  }
  try {
...
apiRouter.use('/admin', requireAdminRole, adminMirrorMiddleware);
...
apiRouter.patch('/admin/users/:id/status', (req, res) => {
  ...
  applyAdminUserStatus(req.params.id, status);
  res.json(result);
});
...
apiRouter.patch('/admin/marketplace/listings/:id/moderate', (req, res) => {
  ...
  applyAdminListingModeration(req.params.id, action);
  res.json(result);
});
