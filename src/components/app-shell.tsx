...
export function LanguageSwitcher() {
  ...
  return (
    <div className="flex items-center gap-1" role="group" aria-label="Language" translate="no">
...
export function AppShell({
  ...
  const handleVoiceTabNavigate = (tab: string) => {
    ...
    } else if (tab === "harvest" || tab === "newBatch") {
      ...
      <header className="sticky top-0 z-30 border-b border-border/70 bg-card/95 backdrop-blur shadow-xs">
        <div className="mx-auto flex w-full max-w-5xl flex-wrap items-center justify-between gap-3 px-4 py-3">
          ...
          <nav className="flex flex-wrap items-center justify-end gap-1.5 text-sm">
            ...
            <Link
              to="/mandi"
              className="whitespace-nowrap rounded-md px-2.5 py-1 text-muted-foreground hover:bg-accent hover:text-foreground font-medium"
            >
              {lang === "mr" ? "थेट बाजारभाव" : lang === "hi" ? "मंडी भाव" : "Live Mandi"}
            </Link>
            ...
            {isUserSignedIn ? (
              <>
                ...
                <Link
                  ...
                  className="whitespace-nowrap rounded-md px-2.5 py-1 text-muted-foreground hover:bg-accent hover:text-foreground font-medium"
                >
                  {lang === "mr" ? "ऑर्डर ट्रॅक" : lang === "hi" ? "ऑर्डर ट्रैक" : "Track Order"}
                </Link>
