...
export function I18nProvider({ children }: { children: React.ReactNode }) {
  ...
  // Strings already translated by our own dictionaries don't need a round trip.
  const known = React.useMemo(() => {
    const set = new Set<string>(Object.values(dicts[lang]));
    if (lang !== "en") for (const m of Object.values(entityTranslations)) if (m[lang]) set.add(m[lang]);
    return set;
  }, [lang]);
  ...
  return (
    <I18nContext.Provider value={value}>
      {children}
      <AutoTranslate lang={lang} known={known} />
