...
import { MandiSnapshot } from "@/components/MandiSnapshot";
...
// Open marketplace listings shown to buyers and processors (live).
function useOpenLots(): BatchRow[] {
  const [lots, setLots] = React.useState<BatchRow[]>([]);
  const load = React.useCallback(async () => {
    ...
    setLots(
      ((data ?? []) as any[]).map((l) => ({
        ...
        crop: l.batches?.crop ?? "Produce",
        quantity_kg: Number(l.quantity_kg),
        status: "listed",
        grade: l.batches?.grade ? (String(l.batches.grade).startsWith("Grade") ? l.batches.grade : `Grade ${l.batches.grade}`) : null,
        grade_score: l.batches?.grade_score ?? null,
        district: l.batches?.district ?? "",
        harvest_date: l.batches?.harvest_date ?? "",
        farmer_name: l.batches?.demo_farmer_name ?? undefined,
...
function Dashboard() {
  ...
  return (
    <AppShell signedIn>
      ...
      {/* Quick Launch Smart Tools Strip (not for processors) */}
      {activeRole !== "processor" && (
          ...
          <Card>
            <CardHeader className="pb-2">
              ...
              <CardDescription className="text-xs">
                Today's modal prices from Agmarknet mandis near you
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-2 text-sm pt-2">
              <MandiSnapshot district={advisory?.weather?.district} />
            </CardContent>
          </Card>
        </div>
      </div>
...
function BuyerDashboardView({
  ...
  const availableLots = useOpenLots();
            ...
            {availableLots.map((b) => (
              ...
              >
                ...
                <div className="text-xs space-y-1 text-muted-foreground">
                  <p>Farmer: <span className="text-foreground font-medium">{b.farmer_name || "Farmer"}</span></p>
                  ...
                  <p>Price: <span className="text-foreground font-semibold text-sm">₹{b.price_per_kg ?? "-"}/kg</span></p>
                </div>
...
function ProcessorDashboardView({
  ...
  // Every open lot, processing grades (B-E) first.
  const openLots = useOpenLots();
  const gradeBLots = [...openLots].sort((a, b) => (a.grade === "Grade A" ? 1 : 0) - (b.grade === "Grade A" ? 1 : 0));
              ...
              >
                ...
                <div className="flex items-center gap-2 pt-2 border-t border-border/50">
                  <Button asChild size="sm" className="h-9">
                    <Link to="/marketplace">Contract Lot at ₹{b.price_per_kg ?? "-"}/kg</Link>
                  </Button>
                  <Button asChild variant="outline" size="sm" className="h-9">
                    <Link to="/trace/$code" params={{ code: b.batch_code }}>{t("traceability")}</Link>
                  </Button>
                </div>
              </div>
            ))}
          ...
          <Card>
            ...
            <CardContent className="text-xs space-y-2 text-muted-foreground">
              ...
              <Button asChild variant="outline" size="sm" className="w-full mt-2">
                <Link to="/marketplace">Browse Farmer Lots</Link>
              </Button>
            </CardContent>
          </Card>
        </div>
