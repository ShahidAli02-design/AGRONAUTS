...
import {
  ...
  hasFullNameForLandCheck,
  type LandVerificationStatus,
} from "@/components/LandRecordVerification";
...
function AuthPage() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    if (mode === "up") {
      ...
      if (role === "farmer" && !hasFullNameForLandCheck(fullName)) {
        setError("Please enter your full name (first name and surname) exactly as on your 7/12 (७/१२ प्रमाणे पूर्ण नाव टाका).");
        return;
      }
      if (role === "farmer" && landStatus === "failed") {
        setError(
          "Your 7/12 (सात-बारा) could not be verified — the name on the document must match the full name you entered."
        );
        return;
      }
    ...
    try {
      ...
      setStoredSession({ user: sessionUser, profile, role: account.role });
      void navigate({ to: account.role === "admin" ? "/admin/dashboard" : "/dashboard" });
    } catch (err) {
