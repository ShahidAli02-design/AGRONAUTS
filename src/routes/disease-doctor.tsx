    ...
    try {
      ...
      if (res.ok) {
        ...
        if (json.success && json.result) {
          ...
          void recordHistory(
            "crop_doctor",
            `${json.result.cropDetected ?? cropHint} · ${json.result.possibleDisease}`,
            `${json.result.isSimulated ? "Offline sample (AI not connected) · " : ""}${json.result.severity} · ${json.result.confidenceScore}% confidence · ${json.result.recommendedAction ?? ""}`.slice(0, 300),
            { symptoms: symptomsList, simulated: json.result.isSimulated },
          );
          return;
        }
      }
