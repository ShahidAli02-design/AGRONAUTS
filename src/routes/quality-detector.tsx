...
import { ToolHistory, recordHistory } from "@/lib/history";
...
export function QualityDetectorPage() {
  ...
  const [decision, setDecision] = React.useState<ZeroWasteDecision | null>(null);
  const [analysisError, setAnalysisError] = React.useState<string | null>(null);
  const [notForSale, setNotForSale] = React.useState<string | null>(null);
  const [seenProduce, setSeenProduce] = React.useState<string | null>(null);

  // A new photo invalidates the previous result, so a stale grade is never
  // shown next to a different image.
  const setNewImage = (img: string | null) => {
  ...
  const runQualityAnalysis = async (customImg?: string) => {
    ...
    try {
      ...
      const localMetrics = await measureProduceImage(imageToAnalyze);
      setSeenProduce(localMetrics?.detectedProduce ?? null);
      ...
      if (data?.notForSale) {
        setReport(null);
        setDecision(null);
        setNotForSale(data.error);
        void recordHistory("quality_grading", `${selectedCrop} · NOT FOR SALE`, data.error, { lot: lotCode });
        return;
      }
      if (!res.ok || !data?.success || !data.qualityReport) {
        throw new Error(data?.error || "Quality grading failed. Please retake the photo and try again.");
      }
      setReport(data.qualityReport);
      setDecision(data.bestUtilization ?? null);
      void recordHistory(
        "quality_grading",
        `${selectedCrop} · ${data.qualityReport.assignedGrade}`,
        `Score ${data.qualityReport.overallScore}/100 · ${lotCode} · ${weightNum} kg · ${data.bestUtilization?.actionTitle ?? data.qualityReport.recommendedUtilization}${data.qualityReport.isSimulated ? " · offline estimate" : ""}`,
        { lot: lotCode, grade: data.qualityReport.assignedGrade, score: data.qualityReport.overallScore },
      );
    } catch (err) {
      console.warn("Quality grading error:", err);
      setReport(null);
      setDecision(null);
      setAnalysisError(
        err instanceof TypeError
          ? "Could not reach the grading server. Check your connection and try again."
          : err instanceof Error
          ? err.message
          : "Quality grading failed. Please try again."
      );
    } finally {
      clearTimeout(t1);
      clearTimeout(t2);
  ...
  return (
    <RequireAuth toolName={lang === "mr" ? "एआय प्रतवारी व गुणवत्ता" : "AI Quality Detector"}>
      <AppShell signedIn={Boolean(user)}>
        ...
        <div className="mt-6 grid gap-6 md:grid-cols-12">
          ...
          <div className="space-y-4 md:col-span-7">
            {analysisError && (
              <div
                role="alert"
                className="flex items-start gap-2 rounded-xl border border-destructive/30 bg-destructive/10 p-3 text-sm"
              >
                <AlertTriangle className="size-4 shrink-0 mt-0.5 text-destructive" />
                <span className="font-semibold text-destructive">{analysisError}</span>
              </div>
            )}
            {notForSale && (
              <Card className="border-2 border-red-500/60 bg-red-500/5">
                <CardContent className="p-8 text-center space-y-3">
                  ...
                  <p className="text-2xl font-black tracking-tight text-red-600">
                    {lang === "mr" ? "विक्रीसाठी नाही" : lang === "hi" ? "बिक्री के लिए नहीं" : "NOT FOR SALE"}
                  </p>
                  <p className="text-sm font-semibold text-foreground">
                    {lang === "mr"
                      ? "या फोटोमध्ये शेतमाल दिसत नाही, त्यामुळे ग्रेड दिला जाणार नाही."
                      : lang === "hi"
                      ? "इस फोटो में उपज नहीं दिख रही, इसलिए ग्रेड नहीं दिया जाएगा।"
                      : "No produce found in this photo, so no grade is given."}
                  </p>
                  <p className="text-xs text-muted-foreground max-w-md mx-auto">{notForSale}</p>
                </CardContent>
              </Card>
            )}
            {report && seenProduce && !selectedCrop.toLowerCase().includes(seenProduce.toLowerCase()) && (
              <div className="flex items-start gap-2 rounded-xl border border-amber-500/40 bg-amber-500/10 p-3 text-xs font-medium text-amber-800 dark:text-amber-300">
                <AlertTriangle className="size-4 shrink-0" />
                <span>
                  {lang === "mr"
                    ? `फोटोमध्ये ${seenProduce} दिसत आहे, पण निवडलेले पीक ${selectedCrop} आहे. अचूक ग्रेडसाठी योग्य पीक निवडा.`
                    : lang === "hi"
                    ? `फोटो में ${seenProduce} दिख रहा है, लेकिन चुनी गई फसल ${selectedCrop} है। सही ग्रेड के लिए सही फसल चुनें।`
                    : `This photo looks like ${seenProduce}, but "${selectedCrop}" is selected. Choose the right crop for an accurate grade.`}
                </span>
              </div>
            )}
            {report?.isSimulated && (
              <div className="flex items-start gap-2 rounded-xl border border-amber-500/30 bg-amber-500/10 p-3 text-xs text-amber-800 dark:text-amber-300">
                ...
                <span>
                  {lang === "mr"
                    ? "AI व्हिजन मॉडेल जोडलेले नाही — हा ग्रेड आपल्या फोटोच्या रंग, डाग व आकार विश्लेषणावरून काढलेला अंदाज आहे."
                    : "AI vision model not connected — this grade is an estimate from colour, blemish and size analysis of your photo. Set GEMINI_API_KEY on the server for full AI grading."}
                </span>
              </div>
            )}
            {notForSale ? null : report ? (
              <Card className="border-border shadow-xs">
                {/* Decision Header */}
        ...
        <ToolHistory tool="quality_grading" />
      </AppShell>
    </RequireAuth>
  );
}
