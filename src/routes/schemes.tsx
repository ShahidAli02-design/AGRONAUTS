...
export function SchemesPage() {
  ...
  return (
    <RequireAuth toolName={lang === "mr" ? "शासकीय योजना व अवजारे" : "Govt Schemes & CHC Rentals"}>
                ...
                <div className="pt-2">
                  <Button asChild size="sm" variant="outline" className="w-full">
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      onClick={() => void recordHistory("scheme", `Applied · ${sch.name}`, `Opened the application portal (${sch.portalUrl})`)}
              ...
              <CardContent className="space-y-3 text-sm flex-1 flex flex-col justify-between">
                ...
                <div className="pt-3">
                  <Button
                    ...
                    className="w-full"
                    onClick={() => {
                      setRentBookingDone(tool.name);
                      void recordHistory("rental", `Booked · ${tool.name}`, `${tool.rate}${(tool as any).hubLocation ? ` · ${(tool as any).hubLocation}` : ""}`);
                    }}
                  >
                    Book for Farm
                  </Button>
      ...
      <ToolHistory tool="scheme" />
      <ToolHistory tool="rental" />
    </AppShell>
    </RequireAuth>
  );
}
