...
export function SoilHealthPage() {
  ...
  return (
    <RequireAuth toolName={lang === "mr" ? "माती परीक्षण व खत मात्रा" : "Soil Health & NPK Calculator"}>
      ...
      <SaveToHistory
        lang={lang}
        onSave={() => recordHistory("soil_health", `${crop} · ${acres} acre · ${district}`, `pH ${ph}, OC ${organicCarbon}%, N ${nitrogen} / P ${phosphorus} / K ${potassium} kg/ha → Urea ${ureaBags}, DAP ${dapBags}, MOP ${mopBags} bags`)}
      />
      <ToolHistory tool="soil_health" />
    </AppShell>
    </RequireAuth>
  );
}
