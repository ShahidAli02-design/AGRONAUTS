...
import { SaveToHistory, ToolHistory, recordHistory } from "@/lib/history";
...
export function YieldPredictorPage() {
  ...
  return (
    <RequireAuth toolName={lang === "mr" ? "उत्पादन व महसूल अंदाज" : "Yield & Revenue Predictor"}>
      ...
      <SaveToHistory
        lang={lang}
        onSave={() => recordHistory("yield_prediction", `${crop} · ${acresNum} acre · ${district}`, `Predicted ${predictedQuintals} q (${predictedTons} t) · ₹${estimatedGrossRevenue.toLocaleString("en-IN")} gross · harvest ~${harvestEstimate} · ${irrigation} irrigation`)}
      />
      <ToolHistory tool="yield_prediction" />
    </AppShell>
    </RequireAuth>
  );
}
