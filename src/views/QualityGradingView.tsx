...
export const QualityGradingView: React.FC<QualityGradingViewProps> = ({
  ...
  const runGrading = async (imageData: string) => {
    ...
    try {
      const localMetrics = await measureProduceImage(imageData);
      ...
      setGradingError(err instanceof Error ? err.message : 'Grading failed. Please try another photo.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-6 pb-12 max-w-5xl mx-auto">
      ...
      {gradingError && !isLoading && (
        <div role="alert" className="rounded-2xl border border-red-800 bg-red-950/60 p-4 text-sm font-semibold text-red-300">
          {gradingError}
        </div>
      )}

      {qualityReport && !isLoading && !gradingError && (
